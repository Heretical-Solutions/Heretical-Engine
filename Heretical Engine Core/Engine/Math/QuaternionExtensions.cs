...
namespace HereticalSolutions.HereticalEngine.Math
{
	...
	public static class QuaternionExtensions
	{
		...
		{
			return CreateQuaternionFromEulerAngles(eulerAngles.X, eulerAngles.Y, eulerAngles.Z);
		}
		...
		public void ToAxisAngle(
			this Quaternion<float> quaternion,
			out Vector3D<float> axis,
			out float angle)
		{
		...
		public Vector4D<float> ToAxisAngle(
			this Quaternion<float> quaternion)
		{
			if (System.Math.Abs(quaternion.W) > 1.0f)
			{
				quaternion = Quaternion<float>.Normalize(quaternion);
			}

			var result = new Vector4D<float>
			{
				W = 2.0f * MathF.Acos(quaternion.W) // angle
			};

			var den = MathF.Sqrt(1.0f - (quaternion.W * quaternion.W));

			if (den > 0.0001f)
			{
				result.Xyz = quaternion.Xyz / den;
			}
			else
			{
				// This occurs when the angle is zero.
				// Not a problem: just set an arbitrary normalized axis.
				result.Xyz = Vector3D<float>.UnitX;
			}
		...
		public void ToEulerAngles(out Vector3D<float> angles)
		{
		...
		public Vector3D<float> ToEulerAngles(
			this Quaternion<float> quaternion)
		{
            //reference
