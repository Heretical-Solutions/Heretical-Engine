		...
		: IModule
	{
		...
		*/
			...
			ApplicationContext context)
		{
			...
			materialDTO.Name = "Quad material";

			materialDTO.ShaderResourceID = "Default shader";

			materialDTO.TextureResourceIDs = new string[]
			{
				TEXTURE_PATH
			};

			string materialResourceID = "Quad/material";
			...
			*/
			...
			string geometryResourceID = "Quad/geometry";
			...
			string meshResourceID = "Quad/mesh";
			...
			foreach (var assetImporter in assetImporters)
			{
				Func<Task> load = async () =>
					{
						logger?.Log<OpenGLDrawTextureModule>($"IMPORTING {assetImporter.GetType().Name} INITIATED");

						await assetImporter
							.Import()
							.ThrowExceptions<IResourceVariantData, OpenGLDrawTextureModule>(context.Logger);

						logger?.Log<OpenGLDrawTextureModule>($"IMPORTING {assetImporter.GetType().Name} FINISHED");
					};
